...
class GetTotalExpensesTool < MCP::Tool
  description "Get total expenses for the past N months. Returns a single number with period label. " \
              "Simple alternative to get_spending_summary when you just need the expense total."
  ...
  class << self
    def call(server_context: nil, **params)
      ...
      total = transactions.sum(:amount).abs.to_f.round(2)
