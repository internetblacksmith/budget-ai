# Builds LLM prompts for various analysis types
class LlmPromptBuilder
  def initialize(analyzer)
    @analyzer = analyzer
  end

  def spending_analysis(options = {})
    period = options[:period] || "this month"
    account = options[:account] || "all accounts"

    transaction_summary = @analyzer.summarize
  ...
  def budget_suggestion(account_stats, monthly_data: [])
  ...
  def transaction_categorization(transaction)
    <<~PROMPT
  ...
  def pattern_explanation(pattern_type)
    case pattern_type
    when :weekend_spending
      build_weekend_spending_prompt
    when :recurring
      build_recurring_prompt
    when :high_value
      build_high_value_prompt
    when :category_spike
      build_category_spike_prompt
    else
      "Explain the #{pattern_type} pattern in the spending data."
    end
  end

  private

  def build_weekend_spending_prompt
  ...
  def build_category_spike_prompt
