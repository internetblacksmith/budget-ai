...
require_relative "../../app/mcp_tools/get_total_expenses_tool"

RSpec.describe GetTotalExpensesTool do
  def call(**params)
  ...
  describe ".call" do
    before do
      ...
      create(:transaction, :expense, amount: -50, date: Date.current)
      create(:transaction, :income, amount: 200, date: Date.current)
    end

    it "returns total expenses for the current month by default" do
      result = call
    ...
    it "looks back multiple months" do
      create(:transaction, :expense, amount: -75, date: 2.months.ago)
      result = call(months_back: 3)
      expect(result["total_expenses"]).to eq(225.0)
    ...
    it "excludes transfers" do
      create(:transaction, :expense, amount: -500, date: Date.current, is_transfer: true)
      result = call
      expect(result["total_expenses"]).to eq(150.0)
    end

    it "excludes income from total" do
      result = call
      expect(result["total_expenses"]).to eq(150.0)
    end

    it "returns zero when no expenses exist in range" do
